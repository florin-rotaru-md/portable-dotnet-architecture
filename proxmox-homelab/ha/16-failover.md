...
## 16.2 Anatomy of an unplanned failover
...
| **QDevice down AND a node dies** | Surviving node has 1/3 votes → **no quorum, no automatic failover** | Until you intervene | ≤ replication interval | `pvecm expected 1` on the survivor (see 16.5) |
...
| **The OS disk fails** | That node is down; HA takes over | ~2-3 min | ≤ replication interval | Reinstall Proxmox on a new OS disk and rejoin — this is a node replacement, follow [17.2](../operations/17-node-replacement.md#172-approach-a--clean-swap-step-by-step) (delnode first, then rejoin; the data pools survive and re-import) |
| **Ransomware / accidental deletion** | Replication faithfully replicates the damage | — | — | **Restore from backup** (Stage 15) — this is why replication isn't backup |
| **Fire, theft, flood** | Both nodes gone | — | — | **Offsite restore** from Digi Storage (Stage 15.2) |

## 16.4 What failover does NOT cover
...
## 16.7 Health checks worth running periodically
