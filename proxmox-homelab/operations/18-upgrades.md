# Stage 17 — Upgrading Postgres (and the pattern for any in-VM upgrade)

*Part of the [waa Proxmox homelab guide](../README.md).*

Everything up to here protects you from hardware failing. This stage is about the other half of the risk: **planned changes you make yourself**. A botched database upgrade takes waa down just as effectively as a dead node, and unlike a dead node it has no automatic recovery — HA can't fail over from a bad schema.
...
## 17.1 Minor and major upgrades are different operations
...
| Mechanism | **Automatic** — `unattended-upgrades` with the PGDG origin (see 17.2) | Playbook installs the new version → `pg_upgradecluster` → playbook again |
...
| Cadence | Within ~24h of publication, unattended | Once a year at most, deliberately, never during an event weekend |
...
## 17.2 Minor upgrades — automatic, by design
...
**Why this is safe to automate — and why majors never are:** the package name embeds the major version. 18.1 → 18.2 is an upgrade of the *same* package, `postgresql-18`; PostgreSQL 19 is a *different package* that no automatic mechanism will ever pull in. The blast radius of automation is structurally bounded to minors. A major happens exactly one way: you bump `postgres_version` and run the playbook (17.3).

**The cost, stated honestly:** installing a minor restarts the cluster — a few seconds, at whatever moment the apt timer fires. In-flight requests during those seconds fail; at ~06:00 against waa's traffic that's a non-event, and Npgsql's pool recovers on the next request. If a deterministic window ever matters, pin it (`systemctl edit apt-daily-upgrade.timer` → `OnCalendar=*-*-* 06:00`) rather than turning automation off.
...
A silently broken unattended-upgrades has the same failure shape as a silently failing replication job ([12.3](../ha/12-ha.md#123-notifications)): everything looks fine until the day it matters. The `--dry-run` check belongs in the same periodic sweep as [15.7](../ha/15-failover.md#157-health-checks-worth-running-periodically).
...
Want ≥ 2× the data directory free. The 640GB disk from [Stage 9](../vms/09-vms.md#resize-the-postgres-disk-after-cloning) makes this a formality at waa's size, but check rather than assume.
...
- **A quiet window.** The database is down for the duration: RSVPs and edits fail, invitations keep serving from Cloudflare ([15.4](../ha/15-failover.md#154-what-failover-does-not-cover)). Announce accordingly.
...
Load the site, submit a real RSVP, edit an event, confirm the row lands. A Postgres that starts is not the same as a Postgres your application works against — and with `Pooling=true` in the connection string, a stale pool produces failures that look like database problems but aren't.
...
⚠️ **Every rollback path discards writes made against the new cluster.** This is why step 3 keeps the application stopped until step 6 passes: while nothing has written, rollback is free. Once RSVPs have landed in PG17, rolling back means losing them, and you're into reconciling by hand.
...
## 17.5 The same pattern, applied elsewhere
...
Doing it the other way — upgrading pve1 first and then trying to migrate onto the un-upgraded pve2 — strands the VMs on one node. Same root cause as the version-skew warning in [13.2](13-maintenance.md#132-returning-a-node-after-a-long-outage-days-to-weeks). For a major PVE release, read the official upgrade notes first; for point releases this is all it takes.
