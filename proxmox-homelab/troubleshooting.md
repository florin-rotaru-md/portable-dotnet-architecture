...
- **`ssh-copy-id` can't seed a key on a new VM** → it needs password auth to install the key, and there is none (above). Push the key from pve1 instead, whose root key is already authorized — Stage 10, "SSH keys".
