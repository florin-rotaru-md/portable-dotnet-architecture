...
What is deliberately **not** here: anything that belongs to Ansible (in-VM state — [the ownership boundary](../operations/20-upgrades.md#205-the-same-pattern-applied-elsewhere)), and unattended versions of `node-return`/`restore-drill` — procedures that move VMs or create them deserve a human watching.
