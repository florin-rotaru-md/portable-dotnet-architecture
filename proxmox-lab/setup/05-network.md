...
The day-to-day consequence is that `corosync-cfgtool -s` shows **link 1 disconnected as its normal state**. `cluster-health` knows this — `ON_DEMAND_LINKS` at the top of the script lists which links are allowed to be down — and classifies it per link, reporting link 1 as expected rather than as a warning. Link 0 is deliberately not in that list: it carries membership, and it being down is an incident. **That is the repo's copy.** The one installed on both nodes is still the 2026-09-04 build, and its failure is not noise — it is silence. It has no such list, and the cron file shipped with it sets no `PATH`, so under cron's `/usr/bin:/bin` it never finds `corosync-cfgtool` at all (the binary is in `/usr/sbin`): its grep for `faulty|disconnected` runs against `command not found`, matches nothing, and takes the else branch, `[ OK ] corosync: all links healthy`. That is what it printed every morning from 2026-09-04 to 2026-09-10 with ring 1 genuinely down ([Stage 7](../cluster/07-cluster.md)), and it would have printed the same had link 0 been the one to go — **a missing tool reads exactly like a healthy ring.** Re-running [2.4](02-post-install.md#24-install-the-helper-scripts-both-nodes) on each node is what starts this check working; until it has been, ring state is something you confirm by hand (5.3).
