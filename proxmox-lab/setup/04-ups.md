...
Two notes at the tail. **The QDevice's battery is why row 4 exists**: a mini PC in that role loses power hard the moment the UPS gives out — survivable, but abrupt — whereas this build's laptop runs on past pve1 and shuts itself down cleanly via the same 3.2 script. It holds nothing but the vote, so either way nothing is lost — the battery just removes an unclean power-off from the routine.
...
`MODE=none` is NUT's own "do not run" switch, and the masks are what make it survive a reboot and an `apt upgrade` of the `nut` packages — `disable` alone doesn't, because `nut-driver-enumerator` re-creates driver instances the moment it runs and the postinst restarts whatever it finds enabled. The template `nut-driver@.service` is masked rather than the `@ups` instance, which covers every instance the enumerator could invent. `/etc/nut` is left in place, and [`pve-config-backup`](../scripts/README.md) sweeps it into the nightly archive, which [`offsite-sync`](../scripts/README.md) uploads to Digi at 05:00 ([17.6](../backup/17-backup-restore.md#176-host-configuration-and-the-ansible-inventory)) — so the config that tells you how to bring NUT back outlives the node that needs it. Also left in place is the stale `/etc/systemd/system/nut-driver@ups.service.d/` drop-in the enumerator generated: inert while the template is masked, regenerated if it ever isn't. [`cluster-health`](../scripts/README.md) reads `MODE` and reports this as the intended state, so a UPS that is off by design never turns into a daily warning nobody can act on — the noise that gets the real ones skimmed past.
