...
`MODE=none` is NUT's own "do not run" switch, and the masks are what make it survive a reboot and an `apt upgrade` of the `nut` packages — `disable` alone doesn't, because `nut-driver-enumerator` re-creates driver instances the moment it runs and the postinst restarts whatever it finds enabled. The template `nut-driver@.service` is masked rather than the `@ups` instance, which covers every instance the enumerator could invent. `/etc/nut` is left in place. [`pve-config-backup`](../scripts/README.md) does sweep it into the nightly archive, but on this build that archive never leaves the node it was made on: no USB drive is attached to either one ([17.2](../backup/17-backup-restore.md#172-backup-storage--the-usb-drive) was never carried out), so the local `mountpoint -q /mnt/usb-backup` fails, and the ship-to-the-peer fallback — retargeted on 2026-09-10 from the on-demand 10G address to `192.168.0.11` over the LAN — finds no drive mounted at the far end either. The only copy is `/var/backups/pve-config` on the single-disk pool you would be rebuilding, so the config that tells you how to bring NUT back dies with the node that needed it. The script does say so, and since 2026-09-10 it says it as a failure: `FAIL: USB drive unreachable — <archive> exists only in /var/backups/pve-config, on the same node and the same single-disk pool it describes`, then `exit 1` — which the cron entry hands to [`infra-report`](../scripts/README.md), so it lands as a red host in the app's admin view instead of in root's mail. **That is the repo's copy, and neither node is running it yet.** Both still carry the 2026-09-04 build until [2.4](02-post-install.md#24-install-the-helper-scripts-both-nodes) is re-run there: it prints `WARN: USB drive unreachable` on stderr and exits 0, still ships to the on-demand `10.10.10.1` rather than the LAN address, and its cron line is unwrapped — so tonight's archive still reports into root's mail, which is refused at the far end (below), which is how a nightly warning went unread for weeks. Also left in place is the stale `/etc/systemd/system/nut-driver@ups.service.d/` drop-in the enumerator generated: inert while the template is masked, regenerated if it ever isn't. [`cluster-health`](../scripts/README.md) reads `MODE` and reports this as the intended state, so a UPS that is off by design never turns into a daily warning nobody can act on — the noise that gets the real ones skimmed past.

**Do not read that silence as a working alarm.** On this build root's mail is written and then thrown away: `MAILTO=root` leaves through `proxmox-mail-forward` and the far end refuses it — `550 5.7.1 Service unavailable ... blocked using Spamhaus`, the residential-sender failure [15.3](../ha/15-ha.md#153-notifications) exists to prevent — after which postfix drops it. `/var/mail/root` and the queue are empty on both nodes; nothing is retained, so there is no backlog to read later either. The same target carries PVE's own `vzdump`, `replication`, `fencing` and `system-mail` notifications, so it is not only this stage that reports into a void. The one channel still delivering is the [`infra-report`](../scripts/README.md) POST to the app (ADR-0015). That is worth knowing precisely here, because it decides where to look: the day the UPS gains a data path and `upsmon` stops answering, `cluster-health` prints `[WARN] power: NUT enabled but not answering — the UPS safety net is offline` and exits non-zero, and the 07:00 run is already wrapped in `infra-report` on both nodes, so that warning turns the host red in the app. It will be seen — just never in root's mail.
