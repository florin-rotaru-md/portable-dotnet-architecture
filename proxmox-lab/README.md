...
- QDevice: Dell Pro 14 (PC14250) — Core Ultra 5 225U, 16GB DDR5, 2TB NVMe, Debian 13, static `192.168.0.10`. Holds the third vote; being a laptop, it also carries its own battery and needs [Stage 3](setup/03-laptop-node.md)'s lid/sleep treatment ([8.1](cluster/08-qdevice.md#81-the-box-and-its-os))
...
| | Covers |
|---|---|
| [Stage 17 — Backup & restore](backup/17-backup-restore.md) | The tiers and what is not backed up (R2), `local` as staging, offsite to Digi Storage through an rclone crypt remote, continuous Postgres WAL with a weekly base and nightly dumps, quarterly and on-demand VM images, host configuration and the Ansible inventory, restore scenarios A–G (G: database point-in-time recovery), post-restore checklist, restore drills |
...
- [ ] **9** Ubuntu template from the cloud image
...
- **Drill day — pre-launch or the quarterly hour:** [Stage 23](operations/23-drill-book.md) is the run order, the pass criteria and the log; the mechanics stay in 14/17/18.
...
- **Media missing from the site, or an R2 object needs to come back:** there is no copy to restore from — the R2 buckets are not backed up ([17.1](backup/17-backup-restore.md#171-the-tiers)); what an app can regenerate comes back by republishing.
