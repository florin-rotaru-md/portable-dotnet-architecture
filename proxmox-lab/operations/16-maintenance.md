...
3. `pvecm status` — quorum OK (the QDevice holds the third vote) — **and `pvesr status`: every job `FailCount` 0, with a recent successful run.** Both are two seconds and neither changes anything, so run them *before* step 1 if the day allows; they are numbered here because this is also the last gate before the shutdown. Quorum tells you the cluster survives the shutdown; only this tells you the data does. Replication is the second copy of every running guest ([Stage 12](../ha/12-replication.md)), and for most of them the only *current* one: the images underneath it are quarterly and on demand ([17.5](../backup/17-backup-restore.md#175-vm-images--quarterly-and-on-demand)), and only the database has a tier measured in minutes ([17.4](../backup/17-backup-restore.md#174-postgres--continuous-wal-and-a-weekly-base)). Two things this catches that quorum cannot:

   - **The guests you chose not to migrate.** Step 2 lets you leave 1023 down rather than move it, which leaves its disks on the machine you are about to open; the only other current copy is whatever its hourly job last put on the peer. If that job has been failing since yesterday, you are about to put a screwdriver inside the enclosure holding the only current copy.
...
⚠️ **While a node is away, the backup tiers stand behind the surviving node, and they cover the database far better than the rest.** They keep running on the node that stayed up: `pg-offsite` acts wherever 1022 runs, and `offsite-sync` and `backup-verify` run on both nodes ([17.1](../backup/17-backup-restore.md#171-the-tiers)). But the returning node's replica is as old as the outage, so for its full length the surviving node holds the only *current* copy of every guest, on the single-device vdevs described above. A second failure inside the window costs the database a minute or two of writes ([17.4](../backup/17-backup-restore.md#174-postgres--continuous-wal-and-a-weekly-base)), and every other guest everything since its last image ([17.5](../backup/17-backup-restore.md#175-vm-images--quarterly-and-on-demand)). That is an argument for finishing the return procedure sooner, not for delaying it: bringing the node back is what recreates the second copy.

⚠️ **And the net that was supposed to catch a job dying quietly is gone as well.** A backup failing for two weeks announces itself by mail to root — which is bounced at the far end every single day on both nodes (`550 5.7.1 … blocked using Spamhaus`) and then dropped by postfix, mail queue empty, `/var/mail/root` not even present, so nothing is retained to read later. It takes PVE's own `vzdump`, `replication` and `fencing` notifications with it, since they leave through the same target: the proof [15.3](../ha/15-ha.md#153-notifications) asks you to run on both nodes is exactly the one that fails today. Until that target is rebuilt, the only channel that reaches you is the [`infra-report`](../scripts/README.md) POST to the app — and it reports only what the copies of `backup-verify` and `cluster-health` installed on the nodes check, which are the ones this stage describes only once [2.4](../setup/02-post-install.md#24-install-the-helper-scripts-both-nodes) has been re-run there (above).
...
1. **QDevice first.** With both nodes up you keep 2 of 3 votes and stay quorate, so the cluster doesn't notice — the QDevice is only the third corosync vote. Read `pvecm status` once it has rejoined and see `Total votes: 3` before step 2: taking pve2 down while the QDevice is still missing leaves pve1 at 1 vote of 3, inquorate.
