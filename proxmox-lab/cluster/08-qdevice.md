...
The QDevice has one job: the cluster's third corosync vote. It is hand-managed like the Proxmox hosts — this stage *is* its documentation. Nothing in `native/infra/ansible` touches this machine, so anything not written here does not exist anywhere.

## 8.1 The box and its OS

This build's QDevice is a **Dell Pro 14 (PC14250)** — Core Ultra 5 225U, 16GB DDR5, 2TB NVMe — with the preinstalled Windows replaced by **Debian 13 (trixie)**, netinst, *standard system utilities* + *SSH server*, no desktop.

**Match the nodes' Debian suite rather than taking the newest:** PVE 9 is built on trixie, so trixie here. Both halves of the qnetd/qdevice pair then come from the same corosync line, `intel-microcode` + `fwupd` install from stock repos exactly as [2.2](../setup/02-post-install.md#22-update-microcode-and-reboot) describes. Ubuntu Server works too and the packages all exist; it just adds snapd and a second update rhythm to the one machine whose entire job is being predictably up. What is *not* an option is anything that keeps Windows underneath: a WSL2 or Hyper-V guest gets NAT'd networking, doesn't start before login, and reboots on a schedule Windows Update chooses — which is the third vote disappearing at random. Dual-boot is the same problem made deliberate.
...
| Partitioning | **Guided – use entire disk**, then **All files in one partition** | Nothing on this box needs its own partition. A separate, politely-sized `/var` is precisely the thing that fills up at 3AM while `df` on `/` still reads 4% |
