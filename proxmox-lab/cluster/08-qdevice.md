...
- **Wired Ethernet and a static IP — `192.168.0.10`** ([0.3](../setup/00-preparation.md#03-network-plan)), configured on the machine and outside the router's DHCP pool. Both nodes must always reach this address, and it gets baked into two places that fail *quietly* when it moves: `corosync.conf` ([8.5](#85-install-and-join)) and 1022's `pg_hba.conf` + UFW as a `/32` ([13.1](../ha/13-wal-stream.md)). A changed address costs you the third vote on **both** nodes at once — the cluster keeps running on 2/2 and looks perfectly healthy until the next time you take a node down ([18.3](../ha/18-failover.md#183-scenario-table)) — and stops the WAL stream, which on these two nodes today surfaces **nowhere**. [`backup-verify`](../scripts/README.md) in the repo does ask: its WAL-slot check queries the primary for `pg_replication_slots`, `[FAIL]`s on a slot it finds inactive and `[FAIL]`s in different words on a VM it could not reach at all, and it runs after the USB-drive block rather than inside it — deliberately, because it reaches VM 1022 over the network and never touches the drive. The copies installed in `/usr/local/sbin` are still the 2026-09-04 ones, whose gate is a bare `exit 0` on any node that is not holding the drive; no drive is attached to *either* node ([17.2](../backup/17-backup-restore.md#172-backup-storage--the-usb-drive)), so on both nodes tonight's run stops before the slot is ever looked at. Until [2.4](../setup/02-post-install.md#24-install-the-helper-scripts-both-nodes) is re-run on each node, a moved address costs you the WAL stream in silence: prove the stream by hand ([13.3](../ha/13-wal-stream.md#133-verify--both-ends-then-end-to-end)) the same day you touch this address. If the chassis has no RJ45, a USB-C/Thunderbolt adapter; don't put quorum on Wi-Fi, where every roam or dropout flaps the third vote and interrupts the WAL stream.
...
**On this build the pve2 half was skipped at install and only closed on 2026-09-10.** Until that afternoon `ssh -o BatchMode=yes root@192.168.0.10 true` answered `Host key verification failed` from pve2 while printing `OK` from pve1 — pve2 had no `/root/.ssh/known_hosts` at all, so it trusted no host key it had not been handed explicitly. It has one now, the QDevice's key is pinned in it, and the `BatchMode` probe returns `OK` from both nodes. One half of the old diagnosis is *not* repaired and is not this stage's to repair: pve2 still has no `/etc/ssh/ssh_known_hosts` symlink into `/etc/pve/priv/known_hosts`, so `ssh root@192.168.0.11` from pve2 has no key to check against — `stricthostkeychecking` is `ask` there, so a human at a terminal gets the *authenticity of host … can't be established* prompt and can wave it through, while anything non-interactive — a script, a cron job, `BatchMode=yes` — fails outright with `Host key verification failed` ([21.7](../operations/21-credentials.md#217-the-fourth-kind-the-pins-nobody-inventories)). PVE's own migration and replication never depended on either file — they pass `-o UserKnownHostsFile=/etc/pve/nodes/<node>/ssh_known_hosts -o HostKeyAlias=<node>` explicitly, and pmxcfs puts those per-node files on both nodes — which is why this gap survives for months looking like nothing at all.

**Doing it on pve1 does nothing for pve2**, and that is structural rather than an oversight — worth knowing because a rebuilt node starts with an empty `known_hosts` and puts you back here. pve1's QDevice entry lives in pve1's *node-local* `/root/.ssh/known_hosts`, which pmxcfs does not replicate. The cluster-wide `/etc/pve/priv/known_hosts` is no help either: on this cluster it holds exactly two lines, `pve1` and `192.168.0.11` — no QDevice line, and not even pve2's own host key. pmxcfs replicates that file faithfully; nothing ever adds to it. PVE's `ssh_merge_known_hosts` has a single caller, the `pvecm create` path (`/usr/share/perl5/PVE/API2/ClusterConfig.pm`), so the creating node's key is merged once at cluster creation and a joining node's key never is. The keys are not the problem either; `authorized_keys` on the QDevice is complete. What was missing was one host key, on one machine.

**What it costs when it is missing** is the symmetric case of [19.2](../operations/19-node-replacement.md) — replacing *pve1*, where pve2 is the survivor driving the rebuild and therefore the node that runs `pvecm qdevice setup 192.168.0.10`. That command's first action, before it so much as looks at the certificate store, is an `ssh-copy-id` to the QDevice ([8.5](#85-install-and-join)), so it fails on its first step with the cluster sitting at one node and one vote, and no margin at all. Nothing catches it sooner: no script and no cron job on pve2 ever opens a session to the QDevice, so the first attempt is the one made under pressure.

**The repair is one interactive connection, on the node that is missing it:** `ssh root@192.168.0.10 true`, comparing the fingerprint against the `ssh-keygen -lf` output taken at the QDevice's own console above, then `yes`. Re-verify with `BatchMode=yes` afterwards — that is the only form that proves key authentication rather than a prompt you answered by hand. Run it again after any node rebuild ([19.4](../operations/19-node-replacement.md#194-verification-after-either-approach)); it is two seconds, and the alternative is finding out during the rebuild of the *other* node.
...
**Then verify what the daemon resolved, not what you wrote** — from a node or your workstation, so the same line also serves as the re-check on any later day (at the QDevice's own console, drop the `ssh` wrapper):

```bash
ssh root@192.168.0.10 'sshd -T | grep -E "^(permitrootlogin|passwordauthentication|kbdinteractiveauthentication)"'
# permitrootlogin without-password      <- sshd's own spelling of prohibit-password
# passwordauthentication no
# kbdinteractiveauthentication no
```

`sshd -T` prints the *effective* configuration, every `Include` resolved in the order sshd itself reads them. `cat`ing the drop-in back only proves you can write to a path; it says nothing about whether the running daemon ever read it. Note the first line: sshd renders `prohibit-password` as `without-password`, so that is a pass, not a failure.

Re-run it on any day you care about the answer. On 2026-09-10 this box answered `passwordauthentication yes` — `/etc/ssh/sshd_config.d/` was empty and the drop-in had never existed on this install, which has been up continuously since 2026-08-16. Nothing re-applies it: Ansible owns nothing on this machine ([8.1](#81-the-box-and-its-os)), and unlike every VM in the lab there is no image default underneath it either — a cloud image gets `PasswordAuthentication no` twice over, from Canonical's `60-cloudimg-settings.conf` and again from the `common` role ([21.4](../operations/21-credentials.md#214-the-console-is-the-final-safety-net--give-it-a-password)). Here the single hand-run heredoc above is the entire mechanism, so losing it is invisible and permanent. Root stayed covered by Debian's `prohibit-password` default, but `devops` — the account [8.2](#82-install-debian-13) has you create, with the password you typed into the installer, and a member of `sudo` — could log in over the network, unfiltered (this box runs no host firewall), on the machine holding the third vote and the whole WAL archive.
...
Dell publishes its business hardware to LVFS, so unlike the generic mini PC that usually fills this role this box really can answer for itself — and `fwupd` gets further here than on either node. `get-devices` prints no `UEFI ESP partition not detected` warning and no `Update Error` against *System Firmware* (verified 2026-09-10), so route A in [16.3](../operations/16-maintenance.md#163-firmware--detect-always-flash-rarely) — `fwupdmgr update`, in-band — is the normal way to take a flash that is actually warranted. On the nodes that same device reads `Not updatable as UEFI ESP partition not detected` and route A refuses at the moment you apply it: detection works on all three machines, flashing in-band works only on this one. What the QDevice does **not** get is a reader of its own. [`cluster-health`](../scripts/README.md) is installed on the two nodes only ([2.4](../setup/02-post-install.md#24-install-the-helper-scripts-both-nodes)) and nothing in the helper set opens a session to `192.168.0.10`, so this box's firmware, its disk, its journal and its cron are read by nobody. Its two *jobs* are covered, from the outside and only as jobs: `cluster-health`'s quorum line counts cluster-wide votes, so a QDevice that stops voting surfaces on **both** nodes as `quorate but only 2/3 votes`, and [`backup-verify`](../scripts/README.md) asks the primary about the replication slot and `[FAIL]`s with `pg-receivewal on the QDevice is down` ([8.1](#81-the-box-and-its-os)) — that second one only from the repo copy, so not until [2.4](../setup/02-post-install.md#24-install-the-helper-scripts-both-nodes) has been re-run on the node. The one schedule this box owns is [13.2](../ha/13-wal-stream.md#132-the-receiver-on-the-qdevice)'s WAL prune, and it has never once executed: `/etc/cron.d/wal-archive-prune` is mode `0664`, cron refuses a job file that is group-writable, and it says so — `(*system*wal-archive-prune) INSECURE MODE (group/other writable)` — 1440 times a day into a journal nobody reads, while the archive still carries segments dated 2026-08-16. `chmod 644 /etc/cron.d/wal-archive-prune` is what cron is asking for, and it is the whole fault: `walarchive`'s `/usr/sbin/nologin` shell is not a second cause, because cron runs an `/etc/cron.d` command under `SHELL` after switching user and never consults the account's passwd field (and `/etc/pam.d/cron` here pulls in no `pam_shells`). Confirm the next run actually deleted something all the same. The point here is that nothing on this box was ever going to tell you. The radar is installed and unwatched: a pending *System Firmware* device on a third of the quorum is seen when you SSH in and look, and not before. Which is not hypothetical — on 2026-09-10 this box was running *System Firmware* 1.16.5 with 1.17.0 sitting on LVFS, a DSA-2026-212 security release, and nothing anywhere had mentioned it. So do the looking where the calendar already sends you — the quarterly firmware sweep across all three boxes in [23.3](../operations/23-drill-book.md#233-the-calendar):

```bash
ssh root@192.168.0.10 fwupdmgr get-updates
```

When a round does come, the QDevice stays the machine you flash **first**: with both nodes up, losing it for a reboot still leaves the cluster quorate — [16.3](../operations/16-maintenance.md#163-firmware--detect-always-flash-rarely) has the full order.
