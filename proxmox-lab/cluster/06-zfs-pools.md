...
Know the alarm path while you are here, because it is thinner than the software suggests: ZFS's own watchdog, `zed`, is active on both nodes with `ZED_EMAIL_ADDR="root"`, so it reports by mailing root and is only as alive as the [15.3](../ha/15-ha.md#153-notifications) notification chain. Prove that chain on both nodes: today root mail is generated correctly on both and then rejected by the recipient (`550 5.7.1 … blocked using Spamhaus`) and discarded, so `zed`'s mail goes nowhere at all. A failing disk then announces itself in exactly one place — `cluster-health`'s nightly `disk:` line, carried out of the node by the `infra-report` POST to the app rather than by mail — and that place is not live on either node yet. The installed copy is still the 2026-09-04 one, which globs `/dev/nvme?n1` and labels its line `nvme:`, and cron's `PATH=/usr/bin:/bin` never reaches `smartctl` in `/usr/sbin`; so what the 07:00 run posts is a `[FAIL] nvme: … SMART health check failed` per NVMe device — one on pve1, three on pve2, all on healthy disks and all manufactured by the missing tool (reproduced on both, 2026-09-10) — while pve1's SATA `apps` disk is never asked at all. The [6.2](#62-what-watches-these-pools-afterwards) fix ends both halves, and only once [2.4](../setup/02-post-install.md#24-install-the-helper-scripts-both-nodes) has been re-run on the node.
