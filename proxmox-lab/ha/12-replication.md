...
| VM | Schedule | Why |
|---|---|---|
| 1022 postgres | `*/1` (every minute) | Live application writes — the data you actually can't afford to lose. The offsite copy of the same writes is WAL archiving to Digi, 1–2 minutes behind ([17.4](../backup/17-backup-restore.md#174-postgres--continuous-wal-and-a-weekly-base)) |
...
Two values rather than four, and the split is deliberate: a minute for 1022, an hour for everything else. It tracks how each guest is *recreated*, not how important it feels: 1022's rows exist nowhere but in its replica and its backups, while 1020 is a `git clone` and a restored key away from being itself again ([Stage 11](../vms/11-bootstrap.md)).
