...
| VM | Schedule | Why |
|---|---|---|
| 1022 postgres | `*/1` (every minute) | Live application writes — the data you actually can't afford to lose |
| 1021 app | `*/5` | Mostly stateless; holds little unique state |
| 1020 control | `*/15` or `*/30` | Tooling only, rebuildable |
| 1023 monitoring | `*/30` | Logs, not source data — losing half an hour of history on a failover is a non-event, and it's the largest disk of the four (Stage 10), so the longest interval keeps its replication traffic proportionate |
