...
| **10G direct cable pulled / NIC dies** | corosync fails over to Link 1 (1G) on its own — but migration and replication do **not**: both are pinned to `10.10.10.0/24` by Migration Settings, so they fail or hang until you repoint them | 0 | 0 | Repoint migration at the LAN and drop the bandwidth limit — [runbook](../troubleshooting.md#runbook--the-10g-link-is-down) — then replace the cable and put both back |
| **Thunderbolt adapter drops off** (pve2) | Same as above — the 10G link disappears, the 1G ring carries the cluster, migration and replication need the same runbook | 0 | 0 | Reseat the adapter; if it recurs, check `dmesg` and the interface name ([troubleshooting](../troubleshooting.md)) |
